...
(unit_op : ∀ x : G, op e x = x)
(inv_op : ∀ x : G, op (inv x) x = e)


open hidden.group


lemma idempotent {G : Type*} [group G] : ∀ x : G, op x x = x → x = e :=
assume x : G,
assume a1 : op x x = x,
let x' := inv x in
calc
x = op e x : by exact eq.symm (unit_op x)
...
lemma op_inv {G : Type*} [group G] : ∀ x : G, op x (inv x) = e :=
assume x : G,
let x' := inv x in
have s1 : op (op x x') (op x x') = op x x', by exact (
  ...
  ... = op x (op e x') : by rw (inv_op x)
  ... = op x x' : by rw (unit_op x')
),
idempotent (op x x') s1


lemma op_unit {G : Type*} [group G] : ∀ x : G, op x e = x :=
assume x : G,
let x' := inv x in
calc
op x e = op x (op x' x) : by rw (inv_op x)
...
-- The unit is unique.
example (G : Type*) [group G] (e' : G) (h_left : ∀ x, op e' x = x) : e' = e :=
have s1 : op e' e' = e', by exact h_left e',
idempotent e' s1


-- The inverse is unique.
example (G : Type*) [group G] (inv' : G → G) (inv_op' : ∀ x : G, op (inv' x) x = e) : ∀ x : G, inv x = inv' x :=
assume x : G,
calc
inv x = op e (inv x) : by exact eq.symm (unit_op (inv x))
... = op (op (inv' x) x) (inv x) : by rw (inv_op' x)
... = op (inv' x) (op x (inv x)) : by exact eq.symm (assoc (inv' x) x (inv x))
... = op (inv' x) e : by rw (op_inv x)
...
let x' := inv x in
have s1 : op x' x = e, by exact inv_op x,
calc
y = op e y : by exact eq.symm (unit_op y)
...
let x' := inv x in
have s1 : op x x' = e, by exact op_inv x,
calc
y = op y e : by exact eq.symm (op_unit y)
...
lemma group_1 {G : Type*} [group G] (x y : G) : op x y = x → y = e :=
assume a1 : op x y = x,
have s1 : x = op x e, by exact eq.symm (op_unit x),
have s2 : op x y = op x e, by exact eq.trans a1 s1,
left_cancel x y e s2


lemma group_2 {G : Type*} [group G] (x y : G) : op x y = e → y = inv x :=
assume a1 : op x y = e,
have s1 : e = op x (inv x), by exact eq.symm (op_inv x),
have s2 : op x y = op x (inv x), by exact eq.trans a1 s1,
left_cancel x y (inv x) s2


lemma group_3 {G : Type*} [group G] (x : G) : x = inv (inv x) :=
have s1 : op (inv x) x = e, by exact inv_op x,
group_2 (inv x) x s1
